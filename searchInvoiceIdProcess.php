...
?>
        <div class="row">
            ...
            <div class="col-2 bg-white d-grid">
                ...
                ?>
                    <button class="btn btn-success fw-bold mt-1 mb-1" onclick="changeStatus('<?php $selected_data['id']; ?>');" id="btn<?php $selected_data['id'] ?>">Confirm Order</button>
                <?php
                } else if ($invoice_data["status"] == 1) {
                ?>
                    <button class="btn btn-warning fw-bold mt-1 mb-1" onclick="changeStatus('<?php $selected_data['id']; ?>');">Packing</button>
                <?php
                } else if ($invoice_data["status"] == 2) {
                ?>
                    <button class="btn btn-info fw-bold mt-1 mb-1" onclick="changeStatus('<?php $selected_data['id']; ?>');">Dispatch</button>
                <?php
                } else if ($invoice_data["status"] == 3) {
                ?>
                    <button class="btn btn-primary fw-bold mt-1 mb-1" onclick="changeStatus('<?php $selected_data['id']; ?>');">Shipping</button>
                <?php
                } else if ($invoice_data["status"] == 4) {
                ?>
                    <button class="btn btn-danger fw-bold mt-1 mb-1 disabled" onclick="changeStatus('<?php $selected_data['id']; ?>');">Delivered</button>
                <?php
                }
                ?>
